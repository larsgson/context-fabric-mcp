...
- Passage text: get_passage. Clause/phrase structure of a passage: one search_constructions call (pattern `clause` + book/chapter parameters), not per-word lookups. Syntactic parent / what a clause or phrase belongs to: call get_word_context(book, chapter, verse, word_index) and take the `node` number of the clause/phrase; then call get_edge_features(node, "mother") (direction "from" = the node's own parent). Empty `edges` means no such relation, e.g. a clause without a mother is an independent main clause: say so plainly. The feature `mother_object_type` only labels a type; it is not the edge. Do not infer parents from a clause list.
